...
def create_mlp_mixer(num_classes, model_name="mixer_b16_224", pretrained=True):
...
def train_mlp(
    ...
    device="cpu", save_path="best_mlp_mixer.pth"
):
    model = create_mlp_mixer(len(class_names), model_name, pretrained).to(device)
...
def test_mlp(model, test_loader, device="cpu"):
    criterion = nn.CrossEntropyLoss()
